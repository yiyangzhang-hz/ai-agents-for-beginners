...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-08-21T14:20:57+00:00",
...
> 簡而言之 - 可以在 MCP 上構建代理到代理的通信系統嗎？答案是肯定的！
...
隨著越來越多的開發者探索具有代理行為的工具（例如運行時間較長、執行過程中可能需要額外輸入等），一個常見的誤解是 MCP 不適合這類場景，主要是因為其早期工具原型專注於簡單的請求-響應模式。

這種觀點已經過時了。過去幾個月，MCP 規範已顯著增強，新增了支持構建長時間運行代理行為的功能：
...
這些功能可以組合使用，從而支持基於 MCP 協議部署的複雜代理和多代理應用。

在本文中，我們將把代理稱為 MCP 服務器上可用的「工具」。這意味著存在一個主應用程序，它實現了一個 MCP 客戶端，該客戶端與 MCP 服務器建立會話並可以調用代理。

## MCP 工具如何具備「代理性」？

在進入實現細節之前，我們先來確定支持長時間運行代理所需的基礎設施能力。
...
<strong>圖 1：</strong>該圖展示了 MCP 代理如何在長時間運行的任務期間向主應用程序流式傳輸實時進度更新和部分結果，使用戶能夠實時監控執行情況。
</div>
...
<div align="center" style="font-style: italic; font-size: 0.95em; margin-bottom: 0.5em;">
<strong>圖 2：</strong>該圖展示了 MCP 的 StreamableHTTP 傳輸和事件存儲如何實現無縫的會話恢復：如果客戶端斷線，它可以重新連接並重放錯過的事件，繼續任務而不丟失進度。
</div>
...
**MCP 支持**：MCP 現在支持工具調用返回資源鏈接類型。當前，一種可能的模式是設計一個工具，該工具創建一個資源並立即返回資源鏈接。工具可以在後台繼續處理任務並更新資源。客戶端則可以選擇輪詢該資源的狀態以獲取部分或完整結果（取決於服務器提供的資源更新），或者訂閱該資源以接收更新通知。

需要注意的是，輪詢資源或訂閱更新可能會消耗資源，並對規模產生影響。社區提案（包括 #992）正在探索服務器可以調用的 webhook 或觸發器，以通知客戶端/主應用程序更新的可能性。

| 功能         | 使用場景                                                                                                                                        | MCP 支持                                                        |
| ------------ | ----------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------- |
| 持久性       | 服務器在數據遷移任務期間崩潰。結果和進度在重啟後仍然存在，客戶端可以檢查狀態並從持久化資源中繼續執行。                                           | ✅ 資源鏈接支持持久存儲和狀態通知                                |

目前，一種常見模式是設計一個工具，該工具創建一個資源並立即返回資源鏈接。工具可以在後台處理任務，發送資源通知作為進度更新或包含部分結果，並根據需要更新資源內容。

<div align="center" style="font-style: italic; font-size: 0.95em; margin-bottom: 0.5em;">
<strong>圖 3：</strong>該圖展示了 MCP 代理如何使用持久資源和狀態通知來確保長時間運行的任務在服務器重啟後仍然可用，允許客戶端檢查進度並檢索結果。
</div>
...
<div align="center" style="font-style: italic; font-size: 0.95em; margin-bottom: 0.5em;">
<strong>圖 4：</strong>該圖展示了 MCP 代理如何在執行過程中交互式地引導人類輸入或請求 AI 幫助，支持複雜的多輪工作流，例如確認和動態決策。
</div>
...
主應用程序本地維護會話 ID 和恢復令牌，使其能夠重新連接到現有會話，而不會丟失進度或狀態。
...
- `research_agent` - 通過採樣進行 AI 總結的研究主題
- `list` - 顯示所有可用工具
- `clean-tokens` - 清除恢復令牌
- `help` - 顯示詳細命令幫助
- `quit` - 退出客戶端
...
- **嘗試實驗**：從簡單的長時間運行任務開始，逐步添加流式傳輸、可恢復性和多代理協調
